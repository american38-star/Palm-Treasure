...
export default {
  ...
  setup() {
    ...
    for (const path in gameModules) {
      ...
      // البحث عن صورة تطابق اسم اللعبة فقط
      for (const imagePath in assetImages) {
        const imageFileName = imagePath.split('/').pop().toLowerCase()
        const imageNameWithoutExt = imageFileName.replace(/\.[^/.]+$/, "")
        
        // التحقق مما إذا كان اسم الصورة يطابق اسم اللعبة بالضبط أو يحتوي عليه
        if (imageNameWithoutExt === fileNameLower || 
            imageNameWithoutExt.includes(fileNameLower) || 
            fileNameLower.includes(imageNameWithoutExt)) {
      ...
      // لا نستخدم أي صورة افتراضية - نترك gameImage = null إذا لم يتم العثور على صورة مطابقة
      if (!gameImage) {
        console.log(`ℹ️ لا توجد صورة مطابقة للعبة ${fileName} - سيتم استخدام الأيقونة فقط`)
      }
      ...
      gamesComponents.push({
        ...
        name: gameName,
        icon: gameIcon,
        image: gameImage, // قد تكون null إذا لم يتم العثور على صورة
  ...
  methods: {
    ...
    handleImageError(event, game) {
      ...
      // التحقق من وجود أيقونة بديلة في نفس الحاوية
      const container = event.target.parentNode
      const existingIcon = container.querySelector('.game-icon')
